...
class ucviTests: XCTestCase {
  ...
  func testUVCI() {
      ...
      XCTAssertNil(try JsonFunctions().applyRule(rule, to: "{}"))
      var data = """
    ...
    XCTAssertEqual(true, try JsonFunctions().applyRule(rule, to: data))
      ...
      data = """
        { "data": "" }
      """
      XCTAssertEqual("", try JsonFunctions().applyRule(rule, to: data))
      data = """
        { "data": "URN:UVCI:01:NL:187/37512422923" }
      """
      XCTAssertEqual("01", try JsonFunctions().applyRule(rule, to: data))
      ...
      data = """
        { "data": "URN:UVCI:01:NL:187/37512422923" }
      """
      XCTAssertEqual("NL", try JsonFunctions().applyRule(rule, to: data))
      data = """
      ...
      """
      XCTAssertEqual("AT", try JsonFunctions().applyRule(rule, to: data))
      data = """
    ...
    data = """
      { "data": "URN:UVCI:01:NL:187/37512422923" }
    """
    XCTAssertEqual("187", try JsonFunctions().applyRule(rule, to: data))
    data = """
      { "data": "URN:UVCI:01:AT:10807843F94AEE0EE5093FBC254BD813#B" }
    """
    XCTAssertEqual("10807843F94AEE0EE5093FBC254BD813", try JsonFunctions().applyRule(rule, to: data))
    data = """
    ...
    data = """
      { "data": "URN:UVCI:01:NL:187/37512422923" }
    """
    XCTAssertEqual("37512422923", try JsonFunctions().applyRule(rule, to: data))
    data = """
      { "data": "01:NL:187/37512422923" }
    """
    XCTAssertEqual("37512422923", try JsonFunctions().applyRule(rule, to: data))
    ...
    data = """
      { "data": "foo/bar::baz#999lizards" }
    """
    XCTAssertEqual("baz", try JsonFunctions().applyRule(rule, to: data))
    data = """
      { "data": "a::c/#/f" }
    """
    XCTAssertEqual("", try JsonFunctions().applyRule(rule, to: data))
    ...
    data = """
      { "data": "foo/bar::baz#999lizards" }
    """
    XCTAssertEqual("999lizards", try JsonFunctions().applyRule(rule, to: data))
    data = """
      { "data": "a::c/#/f" }
    """
    XCTAssertEqual("", try JsonFunctions().applyRule(rule, to: data))
    ...
    data = """
      { "data": "a::c/#/f" }
    """
    XCTAssertEqual("f", try JsonFunctions().applyRule(rule, to: data))
